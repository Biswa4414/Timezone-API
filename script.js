...
function fetchLocation(lat, lon) {
  fetch(
    `https://api.geoapify.com/v1/geocode/reverse?lat=${lat}&lon=${lon}&format=json&apiKey=f3f5f60bdefc4d75a8dacea2a76a9a23`
  )
    .then((resp) => resp.json())
    .then((result) => {
      //   console.log(result);
...
addBtn.addEventListener("click", (event) => {
  ...
  if (address.trim() == "") {
    ...
    fetch(
      `https://api.geoapify.com/v1/geocode/search?text=${address}&apiKey=f3f5f60bdefc4d75a8dacea2a76a9a23`
    )
      .then((resp) => resp.json())
